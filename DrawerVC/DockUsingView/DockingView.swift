    ...
    private var dockingViewState: DockingViewState = .dismissed {
        didSet {
            if dockingViewState == .docked {
                ...
                if DeviceSpecific.DeviceConstants.hasSafeArea {
                    UIApplication.shared.isStatusBarHidden = false
                } else {
...
extension DockingView {
    ...
    class func initialize<T: DockingView>(_ superview: UIView, referenceView: UIView, type: T.Type) -> T? {
        ...
        dView.referenceView = referenceView
        dView.addAsSubViewWithConstraints(superview)
       // dView.topView.backgroundColor = .clear
       // dView.centralView.backgroundColor = .clear
        dView.topViewRatio = 16/9
        return dView as? T
    }
...
private extension DockingView {
    func getFrameOfDockingViewForLeftSwipeDismiss(touchingPoint: CGPoint, viewState: DockingViewState) -> CGRect {
        var dC = (touchStartingPoint?.x ?? 0) - touchingPoint.x
